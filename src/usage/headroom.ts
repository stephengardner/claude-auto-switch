...
export interface UsageLike {
  ...
  /** When each account-wide window resets, so a spent one can be seen to lift. */
...
/**
 * The worst (binding) utilization across every window still in force, or null
 * when nothing is known.
 *
 * Windows whose reset has passed are skipped rather than counted: their number
 * describes a limit that has already lifted, and treating it as live is how an
 * account gets abandoned for a cap it no longer has.
 */
export function bindingUtilization(
  ...
  const values: number[] = [];
  if (typeof usage.fiveHour === 'number' && windowIsOpen(usage.fiveHourReset, now)) {
    values.push(usage.fiveHour);
  }
  if (typeof usage.sevenDay === 'number' && windowIsOpen(usage.sevenDayReset, now)) {
    values.push(usage.sevenDay);
  }
  for (const m of usage.models ?? []) {
    ...
    if (typeof m.utilization === 'number' && windowIsOpen(m.resetsAt, now)) {
      values.push(m.utilization);
    }
  }
...
export function headroom(
  ...
  const used = bindingUtilization(usage, model, now);
...
export interface ProactiveInput {
  ...
  model?: string;
  /** Evaluated against this instant, so a window that has reset is not a limit. */
  now?: number;
}
...
export function decideProactiveSwitch(input: ProactiveInput): ProactiveDecision {
  ...
  const currentUsed = bindingUtilization(input.usage.get(input.current), input.model, now);
  if (currentUsed === null) {
  ...
  for (const c of eligible) {
    const room = headroom(input.usage.get(c.name), input.model, now);
    if (room === null) continue; // never gamble on an unknown account
    if (1 - room >= threshold) continue; // already at or over the threshold itself
